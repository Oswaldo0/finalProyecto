      ...
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm sm:p-6">
        ...
        <div className="mt-4 overflow-x-auto rounded-2xl border border-slate-200 bg-white">
          <table className="min-w-full border-collapse text-sm">
            ...
            <tbody>
              {DOCUMENTOS_ABSORCION.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-4 py-10 text-center text-sm text-slate-500">
                    No hay dictámenes de absorción para mostrar.
                  </td>
                </tr>
              ) : (
                DOCUMENTOS_ABSORCION.map((documento) => (
                  <tr key={documento.id} className="odd:bg-white even:bg-slate-50">
                    ...
                    <td className="border-t border-slate-200 px-4 py-3">{documento.carreraOrigen}</td>
                    <td className="border-t border-slate-200 px-4 py-3">{documento.planSolicitado}</td>
