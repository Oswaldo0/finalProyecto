    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-5 sm:px-6">
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-semibold text-slate-700">Imprimir retiro de ciclo</h2>
            <p className="mt-1 text-sm text-slate-500">
              Aquí se listarán los documentos de retiro de ciclo generados para su impresión.
            </p>
          </div>
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead className="bg-slate-100">
              <tr>
                ...
                <th className="border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">Expediente</th>
                <th className="border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">Alumno</th>
                <th className="border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">Ciclo a retirar</th>
            ...
            <tbody>
              {DOCUMENTOS_RETIRO.length === 0 ? (
                ...
                DOCUMENTOS_RETIRO.map((documento) => (
                  <tr key={documento.id} className="odd:bg-white even:bg-slate-50">
                    ...
                    <td className="border-t border-slate-200 px-4 py-3">{documento.expediente}</td>
                    <td className="border-t border-slate-200 px-4 py-3">{documento.alumno}</td>
                    <td className="border-t border-slate-200 px-4 py-3">{documento.cicloRetirar}</td>
