...
export function StudentCreateModal({
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const optionsResponse = await fetch("/api/estudiantes/form-options");
        ...
        if (mode === "edit" && expediente) {
          const studentResponse = await fetch(
            `/api/estudiantes/${expediente}/edicion`,
          );
          if (!studentResponse.ok) {
            throw new Error("No se pudieron cargar los datos del estudiante.");
          }
          ...
          if (!cancelled) {
            setForm((current) => ({
              ...
              expediente: studentResult.student.expediente || "",
              departamento: studentResult.student.departamento || "",
              municipio: studentResult.student.municipio || "",
              direccion: studentResult.student.direccion || "",
            }));
          }
        }
      } catch (loadError) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const endpoint =
        mode === "edit"
          ? `/api/estudiantes/${encodeURIComponent(expediente)}`
          : "/api/estudiantes";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 px-3 py-4">
      <form
        className="h-[90vh] w-full max-w-4xl overflow-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"
      ...
      >
        ...
        <p className="mt-1 text-sm text-slate-600">
          {mode === "edit"
            ...
            : "Complete los datos del estudiante para crear un registro en MySQL."}
        </p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <Field label="num_carnet">
            <input
              ...
              onChange={(e) => updateField("num_carnet", e.target.value)}
              className="rounded-lg border border-slate-300 px-3 py-2"
            />
          </Field>
          ...
          <Field label="fecha_nac">
            <input
              ...
              onChange={(e) => updateField("fecha_nac", e.target.value)}
              className="rounded-lg border border-slate-300 px-3 py-2"
            />
          </Field>
          <Field label="anio_ingreso">
            <input
              ...
              onChange={(e) => updateField("anio_ingreso", e.target.value)}
              className="rounded-lg border border-slate-300 px-3 py-2"
            />
          </Field>
          <Field label="calidad">
          ...
          <Field label="estado_academico">
            <input
              ...
              onChange={(e) => updateField("estado_academico", e.target.value)}
              className="rounded-lg border border-slate-300 px-3 py-2"
            />
          </Field>
          <Field label="institucion_proc">
            <input
              ...
              onChange={(e) => updateField("institucion_proc", e.target.value)}
              className="rounded-lg border border-slate-300 px-3 py-2"
            />
          </Field>
          <Field label="empleo">
            <input
              value={form.empleo}
              onChange={(e) => updateField("empleo", e.target.value)}
              className="rounded-lg border border-slate-300 px-3 py-2"
            />
          </Field>
          ...
          <Field label="departamento" required>
            ...
            <datalist id="departamentos-list">
              {options.departamentos.map((option) => (
                <option key={option.id} value={option.departamento} />
              ))}
            </datalist>
          </Field>
          ...
          <Field label="municipio" required>
            ...
            <datalist id="municipios-list">
              {options.municipios.map((option) => (
                <option key={option.id} value={option.municipio} />
              ))}
            </datalist>
          </Field>
          ...
          <Field label="direccion" required>
            <input
              ...
              required
              className="rounded-lg border border-slate-300 px-3 py-2"
              placeholder="Ingrese direccion manual"
            />
          </Field>
          <SelectField
            label="id_carrera"
            value={form.id_carrera}
            onChange={(value) => updateField("id_carrera", value)}
          ...
          <SelectField
            ...
            onChange={(value) => updateField("id_responsable", value)}
          ...
          <SelectField
            label="id_ciclo"
            value={form.id_ciclo}
            onChange={(value) => updateField("id_ciclo", value)}
            options={options.ciclos}
            textKey="nombre"
          />
          <SelectField
            label="id_grupo"
            value={form.id_grupo}
            onChange={(value) => updateField("id_grupo", value)}
